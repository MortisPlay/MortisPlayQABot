...
def check_question_meaning(question: str) -> tuple[bool, str]:
    ...
    bot_keywords = ["бот", "telegram", "телега", "телеграм", "bot", "api", "автоматизация", "скрипт", "программирование бота"]
    
    # Проверка на упоминание бота или связанных терминов
    if any(keyword in question_lower for keyword in bot_keywords):
        logger.info(f"Вопрос отклонён: содержит упоминание бота или автоматизации ({question})")
        return False, "Вопросы о боте, его функциях или автоматизации запрещены. Задайте вопрос о контенте Mortis Play!"
    
    # Проверка длины вопроса
    if len(question_lower) < 10:
        return False, "Вопрос слишком короткий (менее 10 символов)."
    
    # Проверка на повторяющиеся символы (буквы, цифры, знаки)
    ...
    # Проверка на подозрительные последовательности цифр (например, длинные числа, коды)
    if re.search(r'\d{6,}', question_lower):
        return False, "Вопрос содержит подозрительные числовые последовательности."
    
    # Проверка на подозрительные символы (например, чрезмерное использование спецсимволов)
    special_chars = re.findall(r'[\W_]', question_lower)
    if len(special_chars) > len(question_lower) / 2:
        return False, "Вопрос содержит слишком много специальных символов."
...
async def guide(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"   • Запрещены: спам, оскорбления, реклама, оффтоп, личная информация, вопросы о боте или его функциях.\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"📢 Вопросы должны быть осмысленными и связанными с контентом Mortis Play. Запрещены вопросы о боте или его функциях!\n"
        f"Новичок? Жми *Гайд* или пиши `/guide`! 🚀"
    )
    try:
